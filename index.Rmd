...
.pull-left[
Logistic Regression is similar to OLS regression with the exception that it is **based on the independent variables prediction of** <span style="color:red">**probabilities, odds, and the logarithm of the odds**</span>**, for a categorical dependent variable $Y$**, rather than the prediction of specific values of a quantitative $Y$
]
...
Note, that in a multiple regression analysis we would be predicting some quantitative variable.
]
...
The <span style="color:red">**probability**</span> of being a satisfy student would be simply the number of satify students (outcome of interest) expressed as a proportion to the total possible $$P(S) = 188 / 400 =.47$$


The <span style="color:red">**odds**</span> would be the ratio of the probability $P(x)$ to its compliment $(1-P(x))$. Hence, the odds of being a satisfy student would be $$P(S) /(1-P(S)) = .47/.53 = .89$$
...
When observing the effect of a unit change in explanatory variable on predicting the outcome of $Y$ we use the logit and we interpret it as the ratio of the odds of being classified in one category of the $Y$ for two different values of the $X_i$.
...
# Binomial Logistic Regression: Typical problems
...
Let us examine one such example using data from Hosmer, Lemeshow, and Sturdivant (2013) on the association between age and coronary disease, available from 👉 https://github.com/TanjaKec/mydata/tree/master/CHDAGE.

```{r, echo=FALSE, fig.align='center'}
knitr::include_graphics("images/CHDAGE.png")
```

Data contains two variables: the age in years (AGE), and presence or absence of evidence of significant coronary heart disease (CHD) for 100 subjects in a hypothetical study of risk factors for heart disease.
---
...
2) we are interested in the analysis of the probability of a presence of CHD based on person's AGE
---
...
The trend is very clear: as age increases, the amount of people diagnosed with coronary disease grows.
...
This summary table clearly shows there is an association between age and coronary disease: the higher the age, the higher the chance to develop coronary diseases.
...
.pull-left[
We observe a positive correlation between age (axis X) and the probability to develop cardiac diseases (axis Y). 
...
Another way of interpreting the paameteres is to is to **estimate the percentage increase** in the chance of the occurrence of $Y$. To do so, one must subtract one unit from the exponentiated regression coefficient and multiply the result by 100, in our case (1.12 - 1) * 100. The increase of a year in person's age is associated with an increase of 12% in the chance of CHD occurring.
...
Lastly, the analyst must validate the results observed with a subsample of
its original dataset. This procedure gives the analysis more reliability,
...
# .shadow-text[To learn more visit: <https://dataliteracy.rbind.io>]

.emphasis[To see it in action visit: <http://covid19sr.rbind.io>]
